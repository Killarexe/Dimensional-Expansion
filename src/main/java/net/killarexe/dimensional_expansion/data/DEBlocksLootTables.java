...
public class DEBlocksLootTables extends BlockLoot {
    ...
    protected void addTables() {
        ...
        add(XP_CROPS.get(), (block) -> {return createCropDrops(XP_CROPS.get(), XP_PLANTS.get(), XP_PLANTS.get(), lBuilderXp); });
        LootItemCondition.Builder lBuilderHealth = LootItemBlockStatePropertyCondition.hasBlockStateProperties(XP_CROPS.get()).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(CropBlock.AGE, 7));
        add(HEALTH_CROPS.get(), (block) -> {return createCropDrops(HEALTH_CROPS.get(), HEART.get(), HEART.get(), lBuilderHealth); });
