...
public class DETags {
	...
	private static TagKey<Block> createBlockTag(String pName) {
		return TagKey.create(Registry.BLOCK_REGISTRY, new ResourceLocation(DEMod.MOD_ID, pName));
	}
	
	private static TagKey<Biome> createBiomeTag(String pName) {
		return TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(DEMod.MOD_ID, pName));
	}
}
